...
const HeroComponent = () => {
  ...
  return (
    <>
      <div className="ml-12 flex max-sm:block">
        {arr.map((e) => {
          return (
            <div key={e.heading}>
      ...
      <div className="mt-12 flex max-sm:block">
        <div>
          {arr2.map((e) => {
            return (
              <div key={e.heading} className="ml-12 mt-6">
                <SideCards heading={e.heading} semiHeading={e.semiHeading} />
              </div>
            );
          })}
        </div>
        <div className="mt-6 ml-5 rounded-xl bg-gray-100 max-sm:ml-1">
          <div className="ml-24 mb-4 flex max-sm:ml-6">
            <svg
